...
Processing blocks are specialized methods built into AutoCleanEEG that extend your tasks with advanced signal processing capabilities. They're scientifically validated, well-documented, and work seamlessly with the core pipeline.
...
class MyTask(Task):
    def run(self):
        self.import_raw()
        self.filter_data()
        self.apply_wavelet_threshold()  # ← Processing block method
        self.create_epochs()
```

**What you get:**
- ✅ Processed data with artifacts removed
- ✅ Comprehensive PDF report showing before/after
- ✅ Processing metadata automatically logged
- ✅ BIDS-compliant derivatives saved

That's it. No complex setup, no external dependencies to manage—just call the method and it works.
...
- `specparam_analysis` - specparam spectral parameterization (aperiodic + periodic, formerly FOOOF)
